...
def run_ansible_galaxy_install():
    try:
        # Запуск команды `ansible-playbook`
        result = subprocess.run(['ansible-galaxy', 'install', '-r', 'requirements.yml', '-p', 'roles'], cwd="./playbook")
    except subprocess.CalledProcessError as e:
        print(f"Error: {e}")    
